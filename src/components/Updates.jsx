...
const Updates = () => {
  return (
    <div className="padding flex justify-between max-heroSection:px-8 max-mobile:px-4 max-tab:flex-col max-tab:items-center max-tab:text-center max-tab:gap-12 max-mobile:gap-8 max-w-[1440px] mx-auto">
      ...
      <div className="flex items-center gap-4 bg-pale-blue rounded-full h-fit py-4 px-4 border-slate-400 border-2 w-fit max-heroSection:gap-0 max-mobile:flex-col max-mobile:bg-transparent max-mobile:border-0 max-mobile:gap-4">
        <input
          ...
          autoComplete='true'
        />
        <Button text={'Subscribe'} needbtn={false} bgInput={'bg-coral-red'} widthNeed={true}/>
      </div>
    </div>
  );
};
